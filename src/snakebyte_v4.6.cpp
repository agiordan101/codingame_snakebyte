...
float evaluate_state(State &state, int player_id)
{
    ...
    for (int i = 0; i < get_player_alive_snake_count(state, player_id); i++)
    {
        ...
        // // int dist = find_closest_energy_cell_manhattan(state, get_snake_head_pos(snake), closest);
        // int dist = cells_to_energy_lookup_table[get_snake_head_pos(snake)][0].distance;
        // if (dist == -1)
...
int main()
{
    ...
    while (true)
    {
        ...
        fprintf(stderr, "States visited this turn: %d\n", beam_search_visited_states_count);
