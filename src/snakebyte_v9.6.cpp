...
float evaluate_state(State &state, int player_id, Pos *snake_targets, bool logging = false)
{
    ...
    int opponent_id = (player_id == map_properties.my_id) ? map_properties.opp_id : map_properties.my_id;
    float opp_reachable_energy_score = 0;
    float opp_support_search_score = 0;

    int opp_snake_count = get_player_alive_snake_count(state, opponent_id);
    for (int i = 0; i < opp_snake_count; i++)
    {
        int snake_id = get_player_alive_snake_id(state, opponent_id, i);
        Snake &snake = get_snake(state, snake_id);
        Pos snake_head_pos = get_snake_head_pos(snake);
        int snake_len = get_snake_body_length(snake);
        ...
        for (int bi = 0; bi < snake_len; bi++)
        {
            ...
            {
                int score = encode_lexicographic_priority(head_to_energy_dist, body_support_to_closest_energy_dist, ratio_between_score_dists);
                opp_reachable_energy_score += 1.0f / score;
            }
            else
            {
                int score = encode_lexicographic_priority(body_support_to_closest_energy_dist, head_to_energy_dist, ratio_between_score_dists);
                opp_support_search_score += 0.2f / score;
            }
    ...
    if (logging)
    {
        ...
        fprintf(stderr, "Opp reachable energy score: %f, Opp support search score: %f\n", opp_reachable_energy_score, opp_support_search_score);
    }
    ...
    return player_points - opponent_points + reachable_energy_score + support_search_score - opp_reachable_energy_score - opp_support_search_score;
